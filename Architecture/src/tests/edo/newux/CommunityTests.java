...
@Ignore  // igb 2018.11.08 --> deactivate CommunityTests in old-LA
@Category(NonAngularLearningArea.class)
public class CommunityTests extends BasicNewUxTest {
	...
	public void testNewCommunityFrontPage() throws Exception {
		...
		report.startStep("Open New Community page and verify title");
		sleep(1);
		if (iteration == 2) homePage.clickToOpenNavigationBar();
		communityPage = homePage.openNewCommunityPage(false);
		communityPage.VerifyCommunityPagesTitle();
	...
	public void testNewMagazineTopicsPage() throws Exception {
		...
		for (int i = 0; i < timesToClickToEnd; i++) {
	...
	private void checkArticlesByLevel(Levels level, String topicName, Boolean checkArticlesPreview, Boolean enterEachArticle) throws Exception {
		...
		*/
		...
		if (checkArticlesPreview) errorListOnTopicPreview = magazinePage.verifyArticlesPreviewInTopicByLevel(level, topicName, errorListOnTopicPreview);
		if (enterEachArticle) errorListOnArticlePage = magazinePage.verifyEveryArticlePageInTopicByLevel(level, topicName, errorListOnArticlePage);
