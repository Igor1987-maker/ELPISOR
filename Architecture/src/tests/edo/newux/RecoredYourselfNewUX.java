...
public class RecoredYourselfNewUX extends SpeechRecognitionBasicTestNewUX {
...
//	static final String defaultUserNameSR = "stud85166839"; -- igb 2018.08.21
	...
	private static final String RECORDINGS_FOLDER = "smb://frontqa2016//ToolsAndResources//SharedUpload//Attachments//"+ PageHelperService.recFolder +"//Recordings//";
	...
	public void testRecordYourselfPanelVocabulary() throws Exception {
		...
		homePage.navigateToCourseUnitAndLesson(courseCodes, "FD", 2, 5);
		learningArea.clickOnStep(2);
		sleep(2);
		
		report.startStep("Click on Recored Yourself Tool and send recording");
		learningArea.clickOnRecordYourselfVocab();
		sleep(2);
	...
	public void testInteract1_FD() throws Exception {
		...
		homePage.navigateToCourseUnitAndLesson(courseCodes, "FD", 2, 2);
		learningArea.clickOnStep(4);
		
		report.startStep("Check current SR instruction");
		//interactSection.checkInstructionText(interactSection.instructionText21, true);
		//interactSection.checkInstructionText(interactSection.instructionText15, true);
		...
		// interactSection.checkStatus(InteractStatus.recorder, 2);
	 	...
	 	/*report.startStep("Check the words level of the recording");
...
*/	 	
		...
		/*report.startStep("Check the 2nd recording");
		words = interactSection.getCurrentSpeakerText(2, textService);
	...
	public void testInteract1_MP4() throws Exception {
		...
		homePage.navigateToCourseUnitAndLesson(courseCodes, "B1", 9, 4);
		learningArea.clickOnStep(4);
		sleep(3);
	...
	public void testInteract1_JPG() throws Exception {
		...
		homePage.navigateToCourseUnitAndLesson(courseCodes, "A2", 1, 3);
	...
	public void testInteract2_FD() throws Exception {
		...
		interactSection.checkInstructionText(interactSection.instructionText4, true);
		interactSection.checkifInteract2StatusChanged(InteractStatus.recorder, 8);
		...
		//recordedText.add(interactSection.getInteract2RecordedText(textService,1));
		//webDriver.printScreen("GotDebugScore");
		//String[] WL = textService.splitStringToArray(interactSection.getWordsScoring("debugScore"));
		//String[] str = interactSection.getCurrentSpeakerText(1, textService);
		//speakerText.add(str);
		//wordLevels.add(WL);
		//interactSection.checkInteract2recorderText(1, speakerText.get(0),	wordLevels.get(0), textService);
		//check WL 
		...
		//sleep(1);
		//webDriver.printScreen("GotDebugScore");
		//WL = textService.splitStringToArray(interactSection.getWordsScoring("debugScore"));
		//wordLevels.add(WL);
		...
		sleep(4);   // SPEAKINGPAL takes a bit longer than VERTO to process.
		interactSection.checkInstructionText(interactSection.instructionText10, true);
		interactSection.clickOnSeeFeedback();
		...
		//report.startStep("Check recoreded text's word levels");
		//interactSection.checkFinalViewWordLevels(recordedText.get(0), wordLevels.get(0), textService, 2);
		//interactSection.checkFinalViewWordLevels(recordedText.get(1), wordLevels.get(1), textService, 4);
		...
		report.report("About to press the Restart button on feedback page");
		webDriver.printScreen();
		interactSection.clickTheRestartButtonOnFeedbackPage();
		report.report("About to check start button status is diabled");
		interactSection.checkThatStartButtonIsDisabled();
...
//			sleep(10);
	...
	public void testRecoredYourselfPanelPronunciation()
			...
			sleep(1);
			learningArea.clickOnPronunciation();
			sleep(2);
			...
			report.startStep("Click on Record and Stop");
			recordPanel.clickOnRecordAndStop(3);
			
			report.startStep("Set progress to first FD course item");
			setProgressInFDToStudent();
			
			/*String[] wordsScoring = textService.splitStringToArray(recordPanel.getWordsScoring("wl"));
	...
	public void testRecordMoreThenEightTimes() throws Exception {
			...
			for (int i = 0; i < numOfRecordingsInTest; i++) {
				...
				// OMZ 11.4.2019 if record wasn't sent correctly, try again instead of failing test
				try {
...
//					System.out.println("index 8");
	...
	@Test
	@TestCaseParams(testCaseID = { "24266","33883" })
	public void testMultipleChoiceSRPractice() throws Exception {
			...
			//sleep(2);
			getUserAndLoginNewUXClass();
			homePage.navigateToCourseUnitAndLesson(courseCodes, "B1", 1, 3);
			sleep(2);
			learningArea.clickOnStep(2);
			sleep(1);
			...
			interactSection.checkSpecificTaskInstructionText(interactSection.taskInstructionText1);
			sleep(1);
			...
			learningArea.verifySRAnswerSelectedWrong(1,3);
			...
			learningArea.clickOnCheckAnswer();
			sleep(2);
			learningArea.verifySRAnswerSelectedCorrect(1,1);
	...
	public void testOpenSpeechPracticeED() throws Exception {
			...
			//studentId = dbService.getUserIdByUserName(defaultUserNameSR, instID);
			...
			//interactSection.checkSpecificTaskInstructionText(interactSection.taskInstructionText2);
			...
			learningArea.checkThatClearToolIsNotDisplayed();
			...
			learningArea.checkThatSeeTextIsDisabled();
			...
			learningArea.validateConfirmModalByMessage(interactSection.openSpeechPanelAlert1, true);
			...
			report.startStep("Verify alert message");
			learningArea.validateAlertModalByMessage(interactSection.openSpeechPanelAlert2, true);
		...
		//--igb 2018.06.21-----------
			...
			assignPage.selectRecordingByCourseUnitAndLessonNameAndGetScore(courseName, unitName, lessonName, 1);
			sleep(1);
	...
	@Test
	@Category(inProgressTests.class) // can be executed only on configured static QA site due to CI issues
	@TestCaseParams(testCaseID = {"39204"})
	public void testStudentRecordingsPageTMS_SPal() throws Exception {
		...
		homePage.navigateToCourseUnitAndLesson(courseCodes, courseCodes[courseIndex], unitNum, lessonNum);
		...
		learningArea.clickOnSeeText();
		sleep(2);
		learningArea.selectTextForRecord(segmentNum);
		sleep(1);
		learningArea.clickOnRecordYourselfSegmentTool();
		sleep(3);
	...
	public void testSetProgressPracticeOpenSpeech() throws Exception {
		...
		report.startStep("Make action go to next step and back and check task status visited & current");
		InteractSection interactSection = new InteractSection(webDriver,testResultService);
		interactSection.clickOpenSpeechStartButton();
		...
		String itemId = pageHelper.getItemIdByCourseUnitLessonStepAndTask(courseId, 9, 2, stepNum, taskIndex+1);
		...
		webDriver.closeAlertByAccept(1);
		sleep(1);
	...
	public void testSetProgressPracticeSpeakingInteract2() throws Exception {
		...
		try {
			...
			learningArea.clickOnStep(5);
			sleep(3);
			learningArea.clickOnInteractStartButton();
			sleep(3);
			
			report.startStep("Move to Step 1 then back to Interact 2 and check is Done");
			...
			report.startStep("Checks the server responsed Done");
			webDriver.refresh();
			learningArea.clickOnStep(5);
			sleep(2);
			learningArea.checkThatTaskIsDone(0);
			...
			String itemId = pageHelper.getItemIdByCourseUnitLessonStepAndTask(courseId, 3, 2, 5, 1);
	...
	public void testSetProgressPracticeSpeakingInteract1() throws Exception {
		...
		try {
			...
			report.startStep("Move to Step 1 then back to Interact 1 and check is Done");
			learningArea.clickOnStep(1);
			sleep(1);
			learningArea.clickOnStep(4);
			sleep(5);
			...
			report.startStep("Check the progress in the DB");
			String courseId = getCourseIdByCourseCode("FD");
			String itemId = pageHelper.getItemIdByCourseUnitLessonStepAndTask(courseId, 3, 2, 4, 1);
	...
	public void testNoSendToTeacherInLessonPreviewFromTMS() throws Exception {
		...
		//--igb 2018.08.14 --- replace code to Init-block
			...
			sleep(1);
			learningArea.clickOnLessoneByIndex(3);
								
			report.startStep("Press See Text, select segment and open Record Panel");		
			learningArea.clickOnSeeText();
			sleep(3);
			learningArea.selectSegmentInSpeakingExploreByNumber(1);
			sleep(3);
			learningArea.clickOnRecordYourselfSegmentTool();
			sleep(3);
	...
	//--igb 2018.08.08 --- replace code to Init-block
		...
		learningArea = homePage.clickOnLesson(1, 2);
